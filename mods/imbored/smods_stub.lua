...
SMODS = SMODS or {}
SMODS._jokers   = {}   -- keyed by full key
SMODS._atlases  = {}   -- keyed by atlas key
SMODS._registered = {} -- keys already registered in G.P_CENTERS
SMODS._card_patched = false
...
-- current_mod: updated per-mod before loading, so atlas paths resolve correctly
SMODS.current_mod = {
    id = "imbored",
    path = "mods/imbored/",
    config = {},
...
-- load_file: loads a file relative to current_mod.path
...
function SMODS.Atlas(t)
    ...
    -- Store the mod path at registration time so we can find the image later
    t._mod_path = SMODS.current_mod and SMODS.current_mod.path or ""
    SMODS._atlases[t.key] = t
    return t
end

function SMODS.Joker(t)
    local mod_id = (SMODS.current_mod and SMODS.current_mod.id) or "imbored"
...
-- Register any newly buffered atlases and jokers into G.P_CENTERS.
-- Safe to call multiple times; only processes entries not yet registered.
function SMODS._register_new()
    ...
    for key, atlas in pairs(SMODS._atlases) do
        if not G.ASSET_ATLAS[key] then
            local base = atlas._mod_path or ""
            -- Try scaled then unscaled path
            for _, path in ipairs({
                base .. "assets/" .. scale_dir .. "/" .. atlas.path,
                base .. atlas.path,
            }) do
    ...
    for key, j in pairs(SMODS._jokers) do
        if not SMODS._registered[key] then
            SMODS._registered[key] = true
            order = order + 1
            ...
            if G.localization and G.localization.descriptions and G.localization.descriptions.Joker and j.loc_txt then
                G.localization.descriptions.Joker[key] = {
                    name = j.loc_txt.name or key, text = j.loc_txt.text or {""}
                }
            end
    ...
    -- Patch Card:calculate_joker once
    if not SMODS._card_patched then
        SMODS._card_patched = true
...
-- _init kept for backwards compat; now just calls _register_new
